    ...
    <div class="addInner thIndex"  :animation="animation">
        ...
        <div class="numSelect">
            ...
            <ul>
                ...
                <li class="nowNum"><input type="number" :value="num" @blur="numnum"></li>
                <li class="addNum" @click="addNum">+</li>
            </ul>
            <ul class="butBtn">
                <li class="addCarButton indexAdd" status="1" @click="addCar(currentNum||goodsInfo.totalCurrentNum)">加入购物车</li>
                <li class="canBuy indexBuy addCarButton" status="2" id="936" goodsid="1438" @click="goRight(currentNum||goodsInfo.totalCurrentNum)">立即下单</li>
            </ul>
        </div>
    </div>
    </div>
...
export default {
    ...
    computed : {
        carList(vue){
            return vue.$store.state.info.cartList.data
        }
    },
    methods : {
        ...
        numnum(e){
            if(e.target.value>this.currentNum){
                utils.alert("库存不足")
        ...
        addCar : function(allnum){
            if(this.choosePrice){
                var carNum = 0;
                this.carList.map((v)=>{
                    if(v.goodsId == this.goodsInfo.id){
                        carNum = v.num
                    }
                })
                if(carNum){
                    if(this.num > allnum-carNum){
                        utils.alert("库存不足(含购物车"+carNum+"件)")
                        return
                    }
                }else{
                    if(this.num > allnum){
    ...
    watch : {
        goodsInfo : function(goodsSku){
            var me = this;
            if(goodsSku.skuInfos&&goodsSku.skuInfos.length){
...
.addInner{position: fixed;left: 0;bottom: 0;width: 100%; background: #fff;z-index: 999;font-size: 14px;transform: translate3d(0,120%,0)}
